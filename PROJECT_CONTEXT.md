...
PaperGrab is a React + Netlify web app for researchers. The user-facing product name is `aCATamic`. The user describes a research topic, and the app browses compact scholarly sources to return:
...
- The visible product name in the UI is `aCATamic`; the repo and Netlify project remain PaperGrab.
- Results order matters: PDFs first, research plan/advice second, reference list last.
- References should appear per paper card with individual copy buttons, not as one combined list.
...
- University Sites is optional through SerpAPI Google JSON and targets compact `site:.edu`, `site:.ac.uk`, and `site:edu.cn` searches.
...
- `src/components/PixelCat.jsx`: 8-bit tabby cat mark and loading animation asset
